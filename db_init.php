 ...
 * used. For details, please see function header comments below
 * 
...
/* 
 * Registers the wp custom post type (cpt) "callrecord"
 * Stored in db table "wp_posts" (where column post_type = "callrecord")
 */
...
/* 
 * Registers the wp taxonomy "length"
 * 1. Taxonomy stored in db table "wp_term_taxonomy"
 * 2. Connections to each "callrecord" post stored in "wp_term_relationships"
 */
function ea_register_taxonomy_length(){
    ...
    /*
     *-"Better be safe than sorry when registering custom taxonomies for custom
     * post types. Use register_taxonomy_for_object_type() right after the function
     * to interconnect them. Else you could run into minetraps where the post type
     * isn't attached inside filter callback that run during parse_request or
     * pre_get_posts."
     * http://codex.wordpress.org/Function_Reference/register_taxonomy
     */
}
...
/* 
 * Registers the wp taxonomy "recommendeddonation"
 * Stored in the same way as taxonomy "length" above
 */
...
// Q: Add app version as well?

/* 
 * Registers the p2p plugin connection type "callrecord_and_empathizer" and
 * "callrecord_and_caller"
 * Connections stored in table "wp_p2p"
 */
